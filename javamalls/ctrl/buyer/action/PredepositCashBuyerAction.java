...
public class PredepositCashBuyerAction {
    ...
    @RequestMapping({ "/store/{storeId}.htm/buyer/buyer_cash.htm" })
    ...
    @RequestMapping({ "/store/{storeId}.htm/buyer/buyer_cash_save.htm" })
    public ModelAndView buyer_cash_save(HttpServletRequest request, HttpServletResponse response,
                                        String id, String currentPage) {
        ModelAndView mv = new JModelAndView("buyer_success.html", this.configService.getSysConfig(),
            this.userConfigService.getUserConfig(), 1, request, response);
        ...
        mv.addObject("url", CommUtil.getURL(request) + "/buyer/buyer_cash_list.htm");

        return mv;
    }

    @SecurityMapping(title = "提现管理", value = "/buyer/buyer_cash_list.htm*", rtype = "buyer", rname = "预存款管理", rcode = "predeposit_set", rgroup = "用户中心")
    @RequestMapping({ "/store/{storeId}.htm/buyer/buyer_cash_list.htm" })
    ...
    @RequestMapping({ "/store/{storeId}.htm/buyer/buyer_cash_view.htm" })
