...
public class TransAreaManageAction {
    ...
    public ModelAndView trans_area_list(HttpServletRequest request, HttpServletResponse response,
                                        String currentPage, String pid, String orderBy,
                                        String orderType) {
        ...
        if ((pid == null) || (pid.equals(""))) {
            ...
            if (parent.getLevel() == 0) {
                ...
                List<TransArea> seconds = this.transareaService.query(
                    "select obj from TransArea obj where obj.parent.id=:pid", map, -1, -1);
            ...
            if (parent.getLevel() == 1) {
                Map<String, Object> map = new HashMap<String, Object>();
                map.put("pid", parent.getId());
                List<TransArea> thirds = this.transareaService.query(
                    "select obj from TransArea obj where obj.parent.id=:pid", map, -1, -1);
                map.clear();
        ...
        CommUtil.saveIPageList2ModelAndView(url + "/admin/trans_area_list.htm", "", params, pList,
            mv);
        List<TransArea> areas = this.transareaService.query(
            "select obj from TransArea obj where obj.parent.id is null", null, -1, -1);
    ...
    public ModelAndView trans_area_save(HttpServletRequest request, HttpServletResponse response,
        ...
        if (areaId != null) {
            ...
            for (String id : ids) {
                ...
                TransArea area = this.transareaService.getObjById(Long.valueOf(Long
    ...
    public String trans_area_del(HttpServletRequest request, String mulitId, String currentPage,
                                 String pid) {
        String[] ids = mulitId.split(",");
        for (String id : ids) {
            if (!id.equals("")) {
                Set<Long> list = genericIds(this.transareaService.getObjById(Long.valueOf(Long
                    .parseLong(id))));
                Map<String, Object> params = new HashMap<String, Object>();
                params.put("ids", list);
                List<TransArea> objs = this.transareaService.query(
                    "select obj from TransArea obj where obj.id in (:ids)", params, -1, -1);
    ...
    public void trans_area_ajax(HttpServletRequest request, HttpServletResponse response,
