...
public class PredepositManageAction {
    ...
    public ModelAndView predeposit_list(HttpServletRequest request, HttpServletResponse response,
                                        ...
                                        String pd_payment, String pd_pay_status, String pd_status,
                                        String pd_userName, String beginTime, String endTime,
        ...
        if (this.configService.getSysConfig().isDeposit()) {
            ...
            PredepositQueryObject qo = new PredepositQueryObject(currentPage, mv, orderBy,
                orderType);
            ...
            if (!CommUtil.null2String(pd_status).equals("")) {
                qo.addQuery("obj.pd_status",
                    new SysMap("pd_status", Integer.valueOf(CommUtil.null2Int(pd_status))), "=");
            }
            if (!CommUtil.null2String(pd_remittance_user).equals("")) {
                qo.addQuery("obj.pd_remittance_user", new SysMap("pd_remittance_user",
                    pd_remittance_user), "=");
            }
            if (!CommUtil.null2String(pd_remittance_bank).equals("")) {
                qo.addQuery("obj.pd_remittance_bank", new SysMap("pd_remittance_bank",
                    pd_remittance_bank), "=");
            }
    ...
    public ModelAndView predeposit_edit(HttpServletRequest request, HttpServletResponse response,
        ...
        if (this.configService.getSysConfig().isDeposit()) {
            if ((id != null) && (!id.equals(""))) {
                Predeposit predeposit = this.predepositService.getObjById(Long.valueOf(Long
                    .parseLong(id)));
    ...
    public ModelAndView predeposit_save(HttpServletRequest request, HttpServletResponse response,
        ...
        if (this.configService.getSysConfig().isDeposit()) {
            ...
            if (predeposit.getPd_status() == 1 && predeposit.getPd_pay_status() == 2) {
                User pd_user = predeposit.getPd_user();
                pd_user.setAvailableBalance(BigDecimal.valueOf(CommUtil.add(
                    pd_user.getAvailableBalance(), predeposit.getPd_amount())));
    ...
    public ModelAndView predeposit_modify(HttpServletRequest request, HttpServletResponse response) {
        ModelAndView mv = new JModelAndView("admin/blue/predeposit_modify.html",
            this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request,
            response);
        if (!this.configService.getSysConfig().isDeposit()) {
            mv = new JModelAndView("admin/blue/error.html", this.configService.getSysConfig(),
    ...
    public void predeposit_user(HttpServletRequest request, HttpServletResponse response,
                                String userName) {
        ...
        if (user != null) {
            ...
            map.put("availableBalance",
                Double.valueOf(CommUtil.null2Double(user.getAvailableBalance())));
    ...
    public ModelAndView predeposit_modify_save(HttpServletRequest request,
                                               ...
                                               String amount, String type, String info,
                                               String list_url) {
        ModelAndView mv = new JModelAndView("admin/blue/success.html",
            this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request,
            response);
