...
import com.javamalls.base.domain.virtual.CglibBean;
...
public class TransportTools {
    ...
    public List<CglibBean> query_all_transprot(String json, int type) throws ClassNotFoundException {
        List<CglibBean> cbs = new ArrayList();
        List<Map> list = (List) Json.fromJson(ArrayList.class, json);
        if ((list != null) && (list.size() > 0)) {
            if (type == 0) {
                for (Map map : list) {
                            ...
                            if (goods_weight > 0.0F) {
                                ...
                                if (trans_add_weight > 0.0F) {
                                    other_price = trans_add_fee
                                                  * (float) Math.round(Math.ceil(CommUtil.subtract(
                        ...
                        if (trans_add_weight > 0.0F) {
                            other_price = trans_add_fee
                                          * (float) Math.round(Math.ceil(CommUtil.subtract(
                                              ...
                                              Float.valueOf(trans_weight)))) / trans_add_fee;
                        }
                        fee += other_price;
                        ...
                        if (city.equals("全国")) {
                            ...
                            if (trans.getTrans_type() == 1) {
                                ...
                                if (goods_weight > 0.0F) {
                                    ...
                                    if (trans_add_weight > 0.0F) {
                                        other_price = trans_add_fee
                                                      * (float) Math.round(Math.ceil(CommUtil
                                                          .subtract(Float.valueOf(goods_weight),
                                                              Float.valueOf(trans_weight))))
                                                      / trans_add_fee;
                                    }
                                    fee += other_price;
                        ...
                        if (goods.getTransport() != null) {
                            mail_fee = mail_fee
                                       + cal_order_trans(goods.getTransport().getTrans_mail_info(),
                                           ...
                                           goods.getGoods_weight(), goods.getGoods_volume(),
                                           city_name);

                            express_fee = express_fee
                                          + cal_order_trans(goods.getTransport()
                                              .getTrans_express_info(), goods.getTransport()
                                              .getTrans_type(), goods.getGoods_weight(),
                                              goods.getGoods_volume(), city_name);
    ...
    private float cal_order_trans(String trans_json, int trans_type, Object goods_weight,
                                  Object goods_volume, String city_name) {
        float fee = 0.0F;
        try {
            boolean cal_flag = false;
            if(trans_json!=null&&!"".equals(trans_json)){
	                        ...
	                        if ((city.equals(city_name)) || (city_name.indexOf(city) == 0)) {
	                            ...
	                            if ((trans_type == 1) && (CommUtil.null2Float(goods_weight) > 0.0F)) {
	                                ...
	                                if (trans_add_weight > 0.0F) {
	                                    other_price = trans_add_fee
	                                                  * (float) Math.round(Math.ceil(CommUtil.subtract(
	                                                      goods_weight, Float.valueOf(trans_weight))))
	                                                  / trans_add_fee;
	                                }
	                                fee += other_price;
	                            }
	                            ...
	                            if (trans_add_weight > 0.0F) {
	                                other_price = trans_add_fee
	                                              * (float) Math.round(Math.ceil(CommUtil.subtract(
	                                                  goods_volume, Float.valueOf(trans_weight))))
	                                              / trans_add_fee;
	                            }
	                            fee += other_price;
	                    ...
	                    for (Map map : list) {
	                        ...
	                        for (String city : city_list) {
	                            if (city.equals("全国")) {
	                                ...
	                                if ((trans_type != 2)
	                                    || (CommUtil.null2Float(goods_volume) <= 0.0F)) {
	                                    break;
	                                }
	                                fee = trans_fee;
	                                float other_price = 0.0F;
