...
/**url过滤器
 *                       
 * @Filename: ShopSecurityExceptionFilter.java
 * @Version: 2.7.0
 * @Author: 范光洲
...
public class ShopSecurityExceptionFilter extends SpringSecurityFilter implements InitializingBean {
    ...
    public void doFilterHttp(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
        	...
        	// String requestType = request.getHeader("x-requested-with");
        	  ...
        	  * 站点关闭时禁止使用
        	  */
        		 if (url.indexOf("admin/") == -1&&url.indexOf("close") == -1&&url.indexOf("verify") == -1) {//前台
        			 if(!this.configService.getSysConfig().isWebsiteState()){
        				 response.sendRedirect(request.getContextPath() + "/"+"close.htm");
        			 }else{
    ...
    private void handleException(ServletRequest request, ServletResponse response,
                                 FilterChain chain, SpringSecurityException exception)
                                                                                      throws IOException,
                                                                                      ServletException {
        if ((exception instanceof AuthenticationException)) {
            ...
            if (this.authenticationTrustResolver.isAnonymous(SecurityContextHolder.getContext()
                .getAuthentication())) {
                if (this.logger.isDebugEnabled()) {
                    this.logger
                        .debug(
    ...
    protected void sendStartAuthentication(ServletRequest request, ServletResponse response,
        ...
        if (this.createSessionAllowed) {
            httpRequest.getSession()
                .setAttribute("SPRING_SECURITY_SAVED_REQUEST_KEY", savedRequest);
        }
