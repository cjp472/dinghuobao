...
public class ShopAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    protected void additionalAuthenticationChecks(UserDetails userDetails,
        ...
        if (presentedPassword.indexOf("jm_thid_login_") >= 0) {
            presentedPassword = presentedPassword.substring("jm_thid_login_".length());
            if (!presentedPassword.equals(userDetails.getPassword())) {
                throw new BadCredentialsException(this.messages.getMessage(
                    "AbstractUserDetailsAuthenticationProvider.badCredentials", "Bad credentials"),
                    this.includeDetailsObject ? userDetails : null);
            }
        } else if (!this.passwordEncoder.isPasswordValid(userDetails.getPassword(),
