...
public class LuceneUtil {
    ...
    public List<LuceneVo> searchIndex(String keyword, int start, int size, double begin_price,
                                      double end_price, Sort sort) throws IOException {
        ...
        try {
            ...
            for (int i = start; i < end; i++) {
                ...
                vo.setVo_goods_collect(CommUtil.null2Int(doc.get("goods_collect")));
    ...
    public LuceneResult search(String params, int pageNo, double begin_price, double end_price,
                               ScoreDoc after, Sort sort) {
...
//                params = "(title:" + params + " OR content:" + params + ")";
            ...
            List<LuceneVo> vo_list = searchIndex(params, (intPageNo - 1) * this.pageSize,
                this.pageSize, begin_price, end_price, sort);
    ...
    private Document builderDocument(LuceneVo luceneVo) {
        ...
        Field id = new Field("id", String.valueOf(luceneVo.getVo_id()), Field.Store.YES,
            Field.Index.ANALYZED);
        Field title = new Field("title", Jsoup.clean(luceneVo.getVo_title(), Whitelist.none()),
            Field.Store.YES, Field.Index.ANALYZED);
        title.setBoost(10.0F);
        Field content = new Field("content",
            Jsoup.clean(luceneVo.getVo_content(), Whitelist.none()), Field.Store.YES,
            Field.Index.ANALYZED);
        ...
        Field add_time = new Field("add_time", CommUtil.null2String(Long.valueOf(luceneVo
            .getVo_add_time())), Field.Store.YES, Field.Index.ANALYZED);
        Field goods_salenum = new Field("goods_salenum", CommUtil.null2String(Integer
            .valueOf(luceneVo.getVo_goods_salenum())), Field.Store.YES, Field.Index.ANALYZED);
        Field goods_collect = new Field("goods_collect", CommUtil.null2String(Integer
            .valueOf(luceneVo.getVo_goods_collect())), Field.Store.YES, Field.Index.ANALYZED);
        document.add(id);
        document.add(title);
    ...
    public static void main(String[] args) {
        ...
        setIndex_path("D:\\eclipse64\\eclipse\\luence\\goods");
        Date d1 = new Date();
        LuceneResult list = lucence.search("2014", 0, 0.0D, 500.0D, null, null);
        Date d2 = new Date();
        ...
        for (int i = 0; i < list.getVo_list().size(); i++) {
            ...
            System.out.println("人气:" + vo.getVo_goods_collect());
        }
