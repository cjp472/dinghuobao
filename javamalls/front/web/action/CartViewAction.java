...
import com.javamalls.front.web.h5.action.H5CartViewAction;
...
public class CartViewAction {
    ...
    @RequestMapping({ "/store/{storeId}.htm/cart_menu_detail.htm" })
     ...
     * @param buy_type
     */
    @RequestMapping({ "/store/{storeId}.htm/add_goods_cart.htm" })
        ...
        if (CommUtil.null2Int(count) > 0) {
            ...
            if (add) {
                ...
                /* if (goods.getInventory_type().equals("spec")) {
                     List<HashMap<String, Object>> list = (List<HashMap<String, Object>>) Json
                     ...
                     for (Map<String, Object> temp : list) {
                         ...
                         if (Arrays.equals(gspIds, temp_ids)) {
                             goodsCount = CommUtil.null2Int(temp.get("count"));
                             goodsPrice = CommUtil.null2Double(temp.get("price"));
                         }
                     }
                 }*/
                ...
                } else if (goods.getGoods_type() == 1) {
                    if (goods.getTotal_weight() != null && !goods.getTotal_weight().equals("")) {
                        spec_info += "<br/>总量:" + goods.getTotal_weight();
                    }
                    if (goods.getType_ratio() != null && !goods.getType_ratio().equals("")) {
                        spec_info += "<br/>类型比例:" + goods.getType_ratio();
                    }
                    if (goods.getColor_ratio() != null && !goods.getColor_ratio().equals("")) {
                        spec_info += "<br/>颜色比例:" + goods.getColor_ratio();
                    }
                    if (goods.getSize_ratio() != null && !goods.getSize_ratio().equals("")) {
                        spec_info += "<br/>尺码比例:" + goods.getSize_ratio();
                    }
                    gc.setSc(sc);
                    gc.setSpec_info(spec_info);
                } else {
                    if (goods.getTotal_weight() != null && !goods.getTotal_weight().equals("")) {
                        spec_info += "<br/>总量:" + goods.getTotal_weight();
                    }
                    if (goods.getSingle_weight() != null && !goods.getSingle_weight().equals("")) {
                        spec_info += "<br/>单份量:" + goods.getSingle_weight();
                    }
                    if (goods.getType_ratio() != null && !goods.getType_ratio().equals("")) {
                        spec_info += "<br/>类型比例:" + goods.getType_ratio();
                    }
                ...
                if (goods.getBind_status() == 2) {
                    for (Goods bindgoods : goods.getBind_goods()) {
                        ...
                        type = "save";
                        /*
                         * sc = new StoreCart(); for (StoreCart sc1 : cart) { if
                         * (sc1.getStore().getId()
                         * .equals(bindgoods.getGoods_store().getId())) { sc = sc1;
                         * type = "update"; break; } }
                         * sc.setStore(bindgoods.getGoods_store()); if (user ==
                         * null) { sc.setCart_session_id(cart_session_id); } else {
                         * sc.setUser(user); } if (((String) type).equals("save")) {
                         * sc.setCreatetime(new Date());
                         * this.storeCartService.save(sc); } else {
                         * this.storeCartService.update(sc); }
                         */
                        gc = new GoodsCart();
                        gc.setCreatetime(new Date());
     ...
     * @param buy_type
     */
    @RequestMapping({ "/store/{storeId}.htm/add_goods_cartAll.htm" })
                            ...
                            if (spec_combination != null && !"".equals(spec_combination)) {//有规格
                                for (GoodsItem item : goods_items) {
                                    ...
                                    if (spec_combination.equals(item.getSpec_combination())) {
                                        ...
                                        if (user != null && strategy != null
                                            && strategy.isDisabled() == false
                                ...
                                Strategy strategy = user.getStrategy();
                                if (user != null && strategy != null
                                    && strategy.isDisabled() == false && strategy.getStatus() == 1) {
                                    //查询策略的货品的价格
                                    Map<String, Object> paramMap = new HashMap<String, Object>();
                                    paramMap.put("strategyId", user.getStrategy().getId());
                                    paramMap.put("goodItemId", item.getId());
    ...
    @RequestMapping({ "/store/{storeId}.htm/remove_goods_cart.htm" })
    ...
    @RequestMapping({ "/store/{storeId}.htm/goods_count_adjust.htm" })
    public void goods_count_adjust(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!"".equals(CommUtil.null2String(spec_id)) && goods != null) {
            ...
            if (CommUtil.null2Int(count) > goodsItem.getGoods_inventory()) {
                count = CommUtil.null2String(goodsItem.getGoods_inventory());
                message = "当前规格库存为：" + count;
            }
        }
        if (cart_type.equals("")) {
                        ...
                        for (int i = 0; i < gcs2.size(); i++) {
                            ...
                            if (gc.getId().toString().equals(cart_id)) {
                                ...
                                /*sc.setTotal_price(BigDecimal.valueOf(
                                        Double.valueOf((CommUtil.null2Int(count))
                                                       * Double.parseDouble(gc.getPrice().toString()))).setScale(2, BigDecimal.ROUND_HALF_EVEN));*/
        ...
        /*   for (Object gc = cart.iterator(); ((Iterator) gc).hasNext();) {
               StoreCart sc = (StoreCart) ((Iterator) gc).next();
               if (sc.getStore().getId().equals(CommUtil.null2Long(store_id))) {
    ...
    @RequestMapping({ "/store/{storeId}.htm/goods_cart1.htm" })
    ...
    @ResponseBody
    @RequestMapping("/store/{storeId}.htm/good_cart_vaild.htm")
    public void good_cart_vaild(HttpServletRequest request, HttpServletResponse response,
                                String store_id) {
        List<StoreCart> cart = cart_calc(request);
        StoreCart sc = null;
        if (cart != null) {
        ...
        if (sc != null) {
            ...
            for (GoodsCart gc : goodCarts) {
                if (!"".equals(CommUtil.null2String(gc.getSpec_id())) && gc.getGoods() != null) { //有规格时

                    List<GoodsItem> goods_item_list = gc.getGoods().getGoods_item_list();
                    String[] gspIds = gc.getSpec_id().split(",");
                    for (GoodsItem temp : goods_item_list) {
                        String[] temp_ids = CommUtil.null2String(temp.getSpec_combination()).split(
                            "_");
                        Arrays.sort(gspIds);
                        Arrays.sort(temp_ids);
                        if (Arrays.equals(gspIds, temp_ids)) {
                        	BigDecimal price = this.goodsViewTools.getStrategyPrice(temp.getGoods(), temp.getId().toString());
                            //if (temp.getGoods_price().compareTo(gc.getPrice()) != 0) {
                        	if (price.compareTo(gc.getPrice()) != 0) {
                                goodNames += "“";
                                goodNames += gc.getGoods().getGoods_name();
                                goodNames += "”";
                            }
                        }
                    }
                    ...
                    //没有规格时
                   // GoodsItem goodsItem = gc.getGoods().getGoods_item_list().get(0);
                    BigDecimal price = this.goodsViewTools.getStrategyPrice(gc.getGoods(), "");
                    //if (goodsItem.getGoods_price().compareTo(gc.getPrice()) != 0) {
                    if (price.compareTo(gc.getPrice()) != 0) {
                        goodNames += "“";
                        goodNames += gc.getGoods().getGoods_name();
                        goodNames += "”";
    ...
    @RequestMapping({ "/store/{storeId}.htm/goods_cart2.htm" })
        ...
        if (sc != null) {
            ...
            Object addrs = this.addressService.query(
                "select obj from Address obj where obj.disabled = 0 "
                        + "and obj.user.id=:user_id order by obj.createtime desc", params, -1, -1);
    ...
    @RequestMapping({ "/store/{storeId}.htm/goods_cart3.htm" })
    ...
    @RequestMapping({ "/store/{storeId}.htm/order_pay_view.htm" })
    ...
    @RequestMapping({ "/store/{storeId}.htm/order_pay.htm" })
    ...
    @RequestMapping({ "/store/{storeId}.htm/order_pay_outline.htm" })
    public ModelAndView order_pay_outline(HttpServletRequest request, HttpServletResponse response,
        ...
        if (pay_session1.equals(pay_session)) {
            ...
            orderFormVo.setCreatetime(of.getCreatetime());
            orderFormVo.setDisabled(of.isDisabled());
            orderFormVo.setOrder_id(of.getOrder_id());
            orderFormVo.setOrder_status(of.getOrder_status());
            orderFormVo.setStore_id(of.getStore().getId());
            orderFormVo.setUser_id(of.getUser().getId());
            String write2JsonStr = JsonUtil.write2JsonStr(orderFormVo);
            sendReqAsync
                .sendMessageUtil(Constant.STORE_ORDERFORM_URL_EDIT, write2JsonStr, "订单线下支付");
            ...
            mv.addObject("op_title", "线下支付提交成功，等待供货商审核！");
    ...
    @RequestMapping({ "/store/{storeId}.htm/order_pay_payafter.htm" })
    public ModelAndView order_pay_payafter(HttpServletRequest request,
        ...
        if (pay_session1.equals(pay_session)) {
            ...
            this.orderFormLogService.save(ofl);
            request.getSession(false).removeAttribute("pay_session");
            mv.addObject("op_title", "货到付款提交成功，等待供货商发货！");
            mv.addObject("url", CommUtil.getURL(request) + "/buyer/order.htm");
        } else {
            mv = new JModelAndView("buyer_error.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
    ...
    @RequestMapping({ "/store/{storeId}.htm/order_pay_balance.htm" })
    ...
    @RequestMapping({ "/store/{storeId}.htm/order_finish.htm" })
    ...
    @RequestMapping({ "/store/{storeId}.htm/cart_address.htm" })
     ...
     */
    @SecurityMapping(title = "地址新增", value = "/cart_address.htm*", rtype = "buyer", rname = "购物流程3", rcode = "goods_cart", rgroup = "在线购物")
    @RequestMapping({ "/store/{storeId}.htm/cart_address_add_ajax.htm" })
    public ModelAndView cart_address_add_ajax(HttpServletRequest request,
                                              HttpServletResponse response, String id) {
        ModelAndView mv = new JModelAndView("cart_address_add_ajax.html",
            this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request,
            response);
        List<Area> areas = this.areaService.query(
            "select obj from Area obj where obj.parent.id is null", null, -1, -1);
        ...
        if (id != null && !"".equals(id)) {
            address = this.addressService.getObjById(Long.valueOf(id));
            Area area = address.getArea();//地区级
            if (area != null && area.getParent() != null && area.getParent().getParent() != null) {
                Map<String, Object> queryMap = new HashMap<String, Object>();
                queryMap.put("parentId", area.getParent().getId());
                areasList = this.areaService.query(
                    "select obj from Area obj where obj.parent.id =:parentId", queryMap, -1, -1);
                queryMap.clear();
                queryMap.put("parentId", area.getParent().getParent().getId());
                cityList = this.areaService.query(
                    "select obj from Area obj where obj.parent.id =:parentId", queryMap, -1, -1);
                proId = area.getParent().getParent().getId() + "";
                cityId = area.getParent().getId() + "";
                areaId = area.getId() + "";

            } else if (area != null && area.getParent() != null
                       && area.getParent().getParent() == null) {
                Map<String, Object> queryMap = new HashMap<String, Object>();
                queryMap.put("parentId", area.getParent().getId());
                cityList = this.areaService.query(
                    "select obj from Area obj where obj.parent.id =:parentId", queryMap, -1, -1);
                proId = area.getParent().getId() + "";
                cityId = area.getId() + "";
            }
    ...
    @RequestMapping({ "/store/{storeId}.htm/cart_address_save.htm" })
    public String cart_address_save(HttpServletRequest request, HttpServletResponse response,
    ...
    @SecurityMapping(title = "购物车中收货地址保存", value = "/cart_address_save.htm*", rtype = "buyer", rname = "购物流程3", rcode = "goods_cart", rgroup = "在线购物")
    @RequestMapping({ "/store/{storeId}.htm/cart_address_save_ajax.htm" })
    public ModelAndView cart_address_save_ajax(HttpServletRequest request,
                                               HttpServletResponse response, String id,
                                               String area_id, String store_id) {
        ModelAndView mv = new JModelAndView("cart_address_ajax.html",
            this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request,
            response);
        WebForm wf = new WebForm();
        Address address = null;
        ...
        if (id.equals("")) {
            ...
            this.addressService.update(address);
            mv = new JModelAndView("cart_address_ajax2.html", this.configService.getSysConfig(),
                this.userConfigService.getUserConfig(), 1, request, response);
        }
    ...
    @RequestMapping({ "/store/{storeId}.htm/order_address.htm" })
    ...
    private void send_sms(HttpServletRequest request, OrderForm order, String mobile, String mark) {
        ...
        try {
            ...
            if ((template != null) && (template.isOpen())) {
                ...
                if ("sms_tobuyer_order_submit_ok_notify".equals(mark)) {
