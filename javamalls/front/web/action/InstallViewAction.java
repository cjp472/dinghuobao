...
public class InstallViewAction implements ServletContextAware {
    ...
    public ModelAndView install(HttpServletRequest request, HttpServletResponse response,
                                String install_status, String title, String pws, String test_data) {
        ...
        if (CommUtil.null2String(install_status).equals("install1")) {
            mv = new JModelAndView("WEB-INF/templates/install/install2.html",
                this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 2,
                request, response);
    ...
    public ModelAndView install_over(HttpServletRequest request, HttpServletResponse response) {
        ...
        mv.addObject("op_title", "您已经安装javamalls商城系统，重新安装请删除install.lock文件");
        return mv;
    }
    ...
    public void install_init_test(HttpServletRequest request, HttpServletResponse response,
                                  String title, String pws) throws Exception {
    ...
    public void install_init_base(HttpServletRequest request, HttpServletResponse response,
                                  String title, String pws) throws Exception {
        String path = request.getSession().getServletContext().getRealPath("/") + "install.lock";
        File file = new File(path);
        if (!file.exists()) {
