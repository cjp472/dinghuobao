var currentPage=2;
$(function(){
	$("#header_search").on('click', '.header-inp', function(){
        $("#keyForm").submit();
    });
	 // 右上侧小导航控件
    $('#header_search').on('click', '#header-nav', function(){
    ...
    $('#header_search').on('click', '.nctouch-nav-layout',function(){
        $('.nctouch-nav-layout').removeClass('show');
    });
    ...
    $("#nav_ul li a").click(function(){
    	...
    	$.ajax({
            type:'post',
            url:domainURL+"/mobile/store_goods_list_ajax.htm?currentPage=1", 
...
function initPage(){
	...
	$.ajax({
			type:'post',
	        url:domainURL+"/mobile/store_goods_list_ajax.htm?currentPage="+currentPage, 
	        data:$("#orderByForm").serialize(),
	        dataType:'html',
			success:function(result){
