...
function getRootPath() {
	...
	if(localhostPaht.indexOf(":") > -1){
		...
		if(projectName == "/buyer" || projectName == "/seller" || projectName == "/goods" || projectName == "/admin"|| projectName == "/store")
			projectName = "";
	}
	return (localhostPaht + projectName);
}
...
function showDialog() {
	...
	var s = "<div id='"
			...
			+ id
			+ "\").remove();'></a><div style='border:3px solid #CCCCCC;'><div class='message_white_iframe' style='margin:0 auto'><h3 class='message_white_title'><span>"
			+ title
...
function closewin(id,confirm_action) {
	...
	if (count == 0) {
		...
		if(confirm_action!="undefined"&&confirm_action!=null&&confirm_action!=""){
			//根据函数名得到函数类型
		      var  func=eval(confirm_action);
		       //创建函数对象，并调用
		      new func();
		}
